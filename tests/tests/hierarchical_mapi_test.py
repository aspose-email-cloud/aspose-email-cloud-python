...
def test_create_mapi(td: EmailApiData):
    ...
    td.api.create_mapi(requests.CreateMapiRequest(
        name,
    ...
    assert td.api.object_exists(requests.ObjectExistsRequest(td.folder + '/' + name, td.storage)).exists
...
def test_mapi_add_attachment(td: EmailApiData):
    ...
    td.api.add_mapi_attachment(requests.AddMapiAttachmentRequest(
        name, attachment, models.AddAttachmentRequest(
            models.StorageFolderLocation(td.storage, td.folder),
            models.StorageFolderLocation(td.storage, td.folder))))
    attachment_downloaded = td.api.get_calendar_attachment(requests.GetCalendarAttachmentRequest(
...
def test_mapi_get_properties(td: EmailApiData):
    name = td.create_calendar()
    properties = td.api.get_mapi_properties(requests.GetMapiPropertiesRequest(
